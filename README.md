...
| Police car (blue, lights) | Rush through any free station except the carwash | None |
...
- **Kid mode** is relaxed: **5 lives**, **+1 life after every finished level** (capped at 5), and a **gentle speed ramp** that only speeds up on quiet levels: it *holds* its speed whenever a new element arrives (carwash, CNG, electric, police), reaching 220 by the finale.
- **Racing mode** is for grown-ups: **3 lives**, no refills, and a **faster ramp** (starts at 150, climbs to 300) using the same hold-on-new-element rule. Each mode's per-level car speed is an explicit `speedByLevel` table that's easy to hand-tune.
...
- `pozadi_2.png` to `pozadi_5.png`: painted backgrounds, one per lane count (2 to 5 lanes)
